...
SNAP_DIR=angular-lotioncat-snapshots
...
do
  ...
  if [[ $i = 0 ]]; then
    ...
    rm -rf $SNAP_DIR/step-$i/app/lotions
  fi

  if [[ $i = 5 ]]; then
    cp -r app/img $SNAP_DIR/step-0/app/
    cp -r app/lotions $SNAP_DIR/step-0/app/
  fi
done
...
zip -r angular-lotioncat-`date +"%y%m%d_%H%M"` $SNAP_DIR
